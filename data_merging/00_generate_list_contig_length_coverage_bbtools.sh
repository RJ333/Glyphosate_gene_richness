...
# add contig length and average coverage of all glyphosate samples to one list 
# and add a column with the sample name  
BASE_DIR=/data/jwerner/glyphosate/IMP/
OUTPUT_DIR=/data/Rene/glyph/

cd $BASE_DIR
# removing old versions if existing
rm -f ${OUTPUT_DIR}/contig_length_coverage.tsv 
...
do
  ...
  awk -v SAMPLE=$sample 'BEGIN {FS = OFS= "\t"} 
    NR > 1 {print SAMPLE, $1, $2, $3}
	' $contig_coverage >> ${OUTPUT_DIR}/contig_length_coverage.tsv
done
